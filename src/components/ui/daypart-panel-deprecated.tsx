...
function DaypartPanelDeprecated({
  onClose,
  onCustomDaypart,
  onBack,
}: DaypartPanelProps) {
  ...
  const handleDayClick = (day: string, event?: React.MouseEvent) => {
    ...
    if (
      event?.shiftKey &&
      lastClickedHeader &&
      lastClickedHeader.type === "day"
    ) {
      // Range select columns
      const startIdx = Math.min(
        days.indexOf(day),
        days.indexOf(lastClickedHeader.value),
      );
  ...
  const handleHourClick = (hour: string, event?: React.MouseEvent) => {
    ...
    if (
      event?.shiftKey &&
      lastClickedHeader &&
      lastClickedHeader.type === "hour"
    ) {
      // Range select rows
      const startIdx = Math.min(
        hours.indexOf(hour),
        hours.indexOf(lastClickedHeader.value),
      );
      const endIdx = Math.max(
        hours.indexOf(hour),
        hours.indexOf(lastClickedHeader.value),
      ...
      const allSelected = hourRowCells.every((cellId) =>
        newSelected.has(cellId),
      );
      if (allSelected) {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected
                            ? "border-primary-600 border-2 bg-yellow-600"
